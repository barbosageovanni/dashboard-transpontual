...
@bp.route('/api/cte/salvar-seguro', methods=['POST'])
@login_required
def api_salvar_cte_seguro():
    """
    ✅ ROTA CORRIGIDA: Salva CTE sem erro 'cannot unpack'
    """
    try:
        dados = request.get_json()
        ...
        if cte_existente:
            # ✅ USAR MÉTODO CORRIGIDO QUE SEMPRE RETORNA TUPLA
        ...
        current_app.logger.error(f"Erro na API salvar CTE seguro: {str(e)}")
        return jsonify({
            'success': False,
            'message': f'Erro interno: {str(e)}'
        }), 500

@bp.route('/api/cte/processar-lote-seguro', methods=['POST'])
@login_required
def api_processar_lote_seguro():
    """
    ✅ ROTA CORRIGIDA: Processa lote sem erro 'cannot unpack'
    """
    try:
        arquivo = request.files.get('arquivo')
        if not arquivo:
        ...
        # Usar ImportacaoService que já está correto
        from app.services.importacao_service import ImportacaoService
        
        # ✅ ImportacaoService.validar_csv_upload sempre retorna tupla
        ...
        # ✅ USAR MÉTODO CORRIGIDO QUE RETORNA DICT
        resultado = CTE.processar_lote_seguro(lista_dados)
        ...
        return jsonify({
            ...
            'detalhes': resultado['detalhes'][:10],  # Limitar detalhes
            'erro_geral': resultado.get('erro_geral')
        })
    
    except Exception as e:
        current_app.logger.error(f"Erro na API processar lote: {str(e)}")
        return jsonify({
            'success': False,
            'message': f'Erro interno: {str(e)}'
        }), 500
