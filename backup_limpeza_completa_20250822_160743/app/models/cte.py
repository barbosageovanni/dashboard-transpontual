...
class CTE(db.Model):
    ...
    def validar_e_salvar(self) -> tuple:
        """
        ✅ MÉTODO CORRIGIDO: Sempre retorna tupla (sucesso, mensagem)
        Substitui qualquer método que retorna apenas bool
        """
        try:
            from app import db
            ...
            if not self.destinatario_nome or not str(self.destinatario_nome).strip():
            ...
            existente = CTE.query.filter_by(numero_cte=self.numero_cte).first()
            if existente and existente.id != getattr(self, 'id', None):
                return False, f"CTE {self.numero_cte} já existe no sistema"
            ...
            db.session.commit()
            return True, f"CTE {self.numero_cte} salvo com sucesso"
            
        except Exception as e:
            db.session.rollback()
            return False, f"Erro ao salvar CTE: {str(e)}"
    
    def atualizar_campos(self, dados: dict) -> tuple:
        """
        ✅ MÉTODO CORRIGIDO: Sempre retorna tupla (sucesso, mensagem)
        """
        try:
            ...
            for campo, valor in dados.items():
                if campo in campos_validos and hasattr(self, campo):
                    try:
                        ...
                        continue  # Pular campos com formato inválido
                    except Exception:
                        continue  # Pular outros erros
            
            if campos_atualizados > 0:
                self.updated_at = datetime.utcnow()
                db.session.commit()
                return True, f"CTE {self.numero_cte} atualizado ({campos_atualizados} campos alterados)"
            else:
                return False, "Nenhum campo foi atualizado"
    ...
    @classmethod
    def processar_lote_seguro(cls, lista_dados: list) -> dict:
        """
        ✅ MÉTODO CORRIGIDO: Processa lote sem erro 'cannot unpack'
        """
        from app import db
        ...
        try:
            for dados in lista_dados:
                ...
                if not numero_cte:
                    resultado['erros'] += 1
                    resultado['detalhes'].append("Item sem número CTE")
                    continue
                ...
                if cte_existente:
                    ...
                    sucesso, mensagem = cte_existente.atualizar_campos(dados)
                    if sucesso:
                        resultado['atualizados'] += 1
                    else:
                        resultado['erros'] += 1
                        resultado['detalhes'].append(f"CTE {numero_cte}: {mensagem}")
                else:
                    # Criar novo CTE
                    novo_cte = cls(
                        numero_cte=numero_cte,
                    ...
                    sucesso, mensagem = novo_cte.validar_e_salvar()
                    if sucesso:
